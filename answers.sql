...
SELECT email FROM salespeople WHERE region = 'Northwest' OR 
region = 'Southwest';
...
SELECT melons.common_name, melons.melon_type, order_items.quantity, order_items.unit_price, order_items.total_price FROM melons JOIN order_items ON (melons.id = order_items.melon_id) WHERE order_id = 2725;
